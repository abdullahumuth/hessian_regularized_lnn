...
class DyT(nn.Module):
    def __init__(self, C, init_α=0.1):
        super().__init__()
...
class DyTCombinedNetwork(nn.Module):
    """
    Wrapper that combines DyT preprocessing with CombinedNetwork.
    This ensures DyT is applied to all inputs consistently.
    """
    def __init__(self,
                 input_size: int,
                 num_layers: int,
        ...
        # DyT preprocessing layer
        self.dyt_layer = DyT(input_size, init_α=init_α)
        
        # Main network
        self.combined_network = CombinedNetwork(
    ...
    def forward(self, x):
        # Apply DyT preprocessing first
        dyt_x = self.dyt_layer(x)
        # Then apply the main network
        return self.combined_network(dyt_x)
...
class LNN(nn.Module):
    def __init__(self, input_size, num_layers, activation_fn, hidden_dim, device, dtype=torch.float32):
        ...
        # Use DyTCombinedNetwork which includes both DyT preprocessing and the main network
        self.network = DyTCombinedNetwork(
            input_size, num_layers, activation_fn, hidden_dim, device, dtype
