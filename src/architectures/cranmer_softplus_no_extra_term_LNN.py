...
#        self.dyt_layer = DyT(input_size).to(self.device, dtype)
        ...
        custom_initialize_weights(self)
    
    def compute_loss(self, pred, target, inputs, lambda_penalty=0.0):
        """
...
#        return self.network(dyt_x)
    ...
    def fit(self, num_epochs_to_run, train_loader, optimizer, scheduler=None, 
            val_loader=None, gradient_clip_norm=1.0, ckpt_path='/kaggle/working/', lambda_penalty=0.0):
...
def custom_initialize_weights(model_instance):
    """
    Applies custom weight initialization to the LNN model instance following
    the variance formula: σ = 1/√n * {2.2 for first layer, 0.58i for hidden layers, n for output layer}
    Only initializes the .mlp part of the network.
    """
    import math
    print("Applying custom variance-based initialization...")
    ...
    with torch.no_grad():
        ...
        # Initialize only the MLP (self.mlp in CombinedNetwork)
        if hasattr(network_to_inspect, 'mlp') and isinstance(network_to_inspect.mlp, torch.nn.Sequential):
            linear_layers = []
            for layer in network_to_inspect.mlp:
                if isinstance(layer, torch.nn.Linear):
                    linear_layers.append(layer)
            
            if len(linear_layers) == 0:
                print("Warning: No linear layers found in MLP.")
                return
                
            print(f"Found {len(linear_layers)} linear layers in MLP.")
            
            for i, layer in enumerate(linear_layers):
                fan_in = layer.weight.size(1)  # Input features
                
                # Apply variance formula from the image
                if i == 0:  # First layer
                    variance_factor = 2.2
                    layer_type = "First layer"
                elif i == len(linear_layers) - 1:  # Output layer
                    variance_factor = fan_in  # n
                    layer_type = "Output layer"
                else:  # Hidden layers
                    variance_factor = 0.58 * (i + 1)  # 0.58i where i is layer index
                    layer_type = f"Hidden layer {i}"
                
                # Calculate standard deviation: σ = variance_factor / √n
                std = variance_factor / math.sqrt(fan_in)
                ...
                print(f"  - {layer_type}: fan_in={fan_in}, variance_factor={variance_factor:.2f}, std={std:.4f}")
        else:
