...
class CombinedNetwork(nn.Module):
    def __init__(self,
        ...
        print(f"CombinedNetwork: Using GELU activation (overriding {activation_fn.__class__.__name__ if hasattr(activation_fn, '__class__') else 'passed activation'})")

        # Build the MLP part - Override activation function to use GELU
        layers = []
        in_feat = input_size
        for _ in range(num_layers - 1):
            layers.append(nn.Linear(in_feat, hidden_dim))
            layers.append(nn.GELU())  # Use GELU activation instead of the passed activation_fn
            in_feat = hidden_dim
...
#        self.dyt_layer = DyT(input_size).to(self.device, dtype)
    ...
    def compute_loss(self, pred, target, inputs, lambda_penalty=100.0):
        """
...
#        return self.network(dyt_x)
    ...
    def fit(self, num_epochs_to_run, train_loader, optimizer, scheduler=None, 
            val_loader=None, gradient_clip_norm=1.0, ckpt_path='/kaggle/working/', lambda_penalty=100.0):
