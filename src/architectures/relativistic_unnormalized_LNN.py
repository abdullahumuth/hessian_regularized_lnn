...
class CombinedNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        qdot_t = x[:, self.n]                 # [B] - time velocity component
        qdot = x[:, self.n+1:]                # [B, input_dim/2 - 1] - spatial velocity components
        # For the quadratic term: -t_dot^2 + (spatial_velocities)^2
        quad_out = self.a * (-qdot_t**2 + (qdot**2).sum(dim=1))  # [B]
        quad_out = quad_out.unsqueeze(-1)    # [B, 1] to match qmlp_out
        out = qmlp_out + quad_out  # [B,1]
...
#        self.dyt_layer = DyT(input_size).to(self.device, dtype)
    ...
    def compute_loss(self, pred, target, inputs, lambda_penalty=1000.0, epsilon=1e-8):
        """
        Computes loss with a PROPORTIONAL and SCALED penalty for a Lorentzian signature.
        This version avoids the "saturating" 1.0 penalty value.
        """
        ...
        metric = hess[:, self.n:, self.n:]
        _B, n, _ = metric.shape

        # --- Define a stable, characteristic scale for each matrix diagonal entry ---
        # diag_abs has shape [B, n] where n is the size of metric (timelike + spatial dims)
        # Use a robust per-entry statistic (median across the batch) so a large φφ won't drown others.
        diag_abs = torch.abs(torch.diagonal(metric, dim1=-2, dim2=-1))  # [B, n]

        # robust per-entry characteristic scale (shape [n])
        # median reduces sensitivity to outliers (very large r values)
        char_by_entry = torch.median(diag_abs, dim=0).values  # shape [n]
        char_by_entry = char_by_entry.detach() + (epsilon)  # avoid zeros

        # Now use char_by_entry to normalize penalties:
        # - timelike penalty will be divided by char_by_entry[0]
        # - spatial-minor determinant penalties will be divided by product(char_by_entry[1:k+1]) (consistent with determinant scaling)
    

        penalties = []

        # --- Step 1: Test the Timelike Direction (H₀₀ ≤ 0) ---
        h_00 = metric[:, 0, 0]

        # Penalize the unscaled positive part of h_00.
        # Normalize by the characteristic scale to balance with other penalties.
        timelike_violation = torch.relu(h_00)
        scaled_timelike_penalty = timelike_violation / (char_by_entry[0] + epsilon)
        penalties.append(scaled_timelike_penalty)

        # --- Step 2: Test the Spacelike Subspace (must be positive-definite) ---
        spatial_metric = metric[:, 1:, 1:]

        for k in range(1, n): # Loop from k=1 to n-1 for the n-1 spatial minors
            spatial_minor_k = spatial_metric[:, :k, :k]
            determinants_k = torch.linalg.det(spatial_minor_k)

            # Penalize the unscaled negative part of the determinant.
            spatial_violation = torch.relu(-determinants_k)

            # Normalize by the characteristic scale raised to the power of the minor's size (k).
            # This correctly accounts for the geometric scaling of determinants.
            # For spatial minors, use indices 1 through k (since spatial dims start at index 1)
            scaled_spatial_penalty = spatial_violation / (torch.prod(char_by_entry[1:1+k]) + epsilon)
            penalties.append(scaled_spatial_penalty)

        # --- Step 3: Combine Penalties ---
        stacked_penalties = torch.stack(penalties, dim=1)
        summed_penalty_per_item = torch.sum(stacked_penalties, dim=1)
        penalty = torch.mean(summed_penalty_per_item)
...
#        return self.network(dyt_x)
    ...
    def fit(self, num_epochs_to_run, train_loader, optimizer, scheduler=None, 
            val_loader=None, gradient_clip_norm=1.0, ckpt_path='/kaggle/working/', lambda_penalty=1000.0):
