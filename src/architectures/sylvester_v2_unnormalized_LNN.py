...
#        self.dyt_layer = DyT(input_size).to(self.device, dtype)
    ...
    def compute_loss(self, pred, target, inputs, lambda_penalty=1000.0, epsilon=1e-8):
        """
        Computes loss with a PROPORTIONAL and SCALED penalty for positive-definiteness,
        based on a robust implementation of Sylvester's Criterion. This is the final,
        recommended version for classical systems.
        ...
        Args:
            ...
            lambda_penalty: Weight for the penalty term
        ...
        A = hess[:, self.n:, self.n:]  # Mass matrix of shape (B, n, n)
        _B, n, _ = A.shape

        # --- Define a stable, characteristic scale for each matrix in the batch ---
        # This is based on the mean of the absolute values of the diagonal elements
        # of the full mass matrix. This gives a single, consistent scale factor.
        diag_abs = torch.abs(torch.diagonal(A, dim1=-2, dim2=-1))
        char_by_entry = torch.median(diag_abs, dim=0).values  # shape [n]
        char_by_entry = char_by_entry.detach() + (epsilon)  # avoid zeros

        penalties = []
        # Loop through all leading principal minors from k=1 to n.
        for k in range(1, n + 1):
            minor_k = A[:, :k, :k]
            determinants_k = torch.linalg.det(minor_k)

            # --- Calculate the raw, unscaled violation ---
            # A violation occurs if the determinant is not positive.
            # relu(-x) is > 0 only if x < 0.
            violation = torch.relu(-determinants_k)

            # --- Normalize the violation to make it scale-invariant ---
            # We divide by the characteristic scale raised to the power of the minor's
            # dimension (k) to account for the natural scaling of determinants.
            # Add epsilon for numerical safety.
            scaled_penalty_k = violation / (torch.prod(char_by_entry[:k]) + epsilon)
            penalties.append(scaled_penalty_k)
        ...
        # 5. Return components for detailed logging
        loss_components = {
